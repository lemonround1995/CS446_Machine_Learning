...
class LinearRegression(LinearModel):
    ...
    def total_loss(self, f, y):
        ...
        """
        sum_square_loss = 0.5 * np.sum(np.square(f - y))
        regularization_term = 0.5 * self.w_decay_factor * np.sum(np.square(self.w))
        total_loss = sum_square_loss + regularization_term
